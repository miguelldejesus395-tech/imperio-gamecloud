...
const PORT=Number(process.env.PORT||8080),ROOT=__dirname,DB=path.join(ROOT,'data','users.json');
const ADMIN_USER=(process.env.ADMIN_USER||'').trim().toLowerCase(),ADMIN_PASSWORD=process.env.ADMIN_PASSWORD||'';
const sessions=new Map(),resetTokens=new Map(),attempts=new Map();

fs.mkdirSync(path.dirname(DB),{recursive:true});
let users={};
try{users=JSON.parse(fs.readFileSync(DB,'utf8'))}
...
function verify(p,u){
let a=Buffer.from(digest(p,u.salt),'hex'),b=Buffer.from(u.hash,'hex');
...
if(Date.now()-r.at>60000)r={n:0,at:Date.now()};
r.n++;
attempts.set(ip,r);
return r.n>40
}
...
return json(res,200,{ok:true,version:'4.0.0'});
...
try{
let b=req.method==='POST'?await read(req):{},
s=session(req),
email=String(b.email||'').trim().toLowerCase();

if(pathname==='/api/users/register'&&req.method==='POST'){
if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)||String(b.password||'').length<8)
return json(res,400,{error:'Informe e-mail válido e senha com 8 caracteres ou mais'});
...
crypto.createHash('sha256').update(String(b.password||'')).digest(),
crypto.createHash('sha256').update(ADMIN_PASSWORD).digest()
)
)
role='admin';
else if(users[email]&&verify(String(b.password||''),users[email]))
role='player';
else
...
role,
email:emailFound,
exp:Date.now()+(b.remember?30*86400000:8*3600000)
});
...
if(!s)return json(res,401,{error:'Entre na sua conta'});

return json(res,200,{
role:s.role,
email:s.email,
name:s.role==='admin'?'Administrador':users[s.email]?.name||'Jogador',
...
resetTokens.set(token,{email,exp:Date.now()+15*60000});
console.log('RECUPERAÇÃO LOCAL (configure e-mail antes de publicar): /?reset='+token)
}
...
let r=resetTokens.get(String(b.token||''));

if(!r||r.exp<Date.now()||String(b.password||'').length<8)
return json(res,400,{error:'Link inválido, expirado ou senha curta'});
...
if(!s||s.role!=='admin')
return json(res,403,{error:'Acesso exclusivo do administrador'});
...
let u=users[email],minutes=Number(b.minutes);

if(!u)return json(res,404,{error:'Jogador não encontrado'});

if(!Number.isSafeInteger(minutes)||minutes<=0||minutes>100000)
return json(res,400,{error:'Informe de 1 a 100000 minutos'});

u.minutes+=minutes;
save();

return json(res,200,{ok:true,minutes:u.minutes})
}
}

return json(res,404,{error:'Função não disponível'});

}catch(e){
console.error(e);
return json(res,400,{error:'Não foi possível processar o pedido'})
}
});

server.listen(PORT,'0.0.0.0',()=>{
console.log('Império GameCloud: http://localhost:'+PORT)
});
